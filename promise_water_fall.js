...
const overrideMethod = (methodName) => {
  override(drone, methodName, original => {
    return () => {
      drone.promises.push(() => new Promise(original));
      if (drone.promises.length === drone.totalChain) {
        drone.promises.reduce((promise, func) => promise.then(func), Promise.resolve());
      }
      return drone;
    }
  });
...
drone.promises = [];
drone.totalChain = 0;
drone.setTotalChain = (total) => drone.totalChain = total;
overrideMethod('takeoff');
overrideMethod('turnOnCamera');
overrideMethod('pointDownGimbal');
overrideMethod('flyToMission');
overrideMethod('takePhoto');
overrideMethod('land');

drone.setTotalChain(6);
